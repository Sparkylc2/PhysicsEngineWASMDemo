...
namespace Collisions
{
    ...
    CollisionResult intersect_polygon(const Rigidbody& rb_a, const Rigidbody& rb_b);
    CollisionResult intersect_circle(const Rigidbody& rb_a, const Rigidbody& rb_b);
    CollisionResult intersect_circle_polygon(const Rigidbody& rb_a, const Rigidbody& rb_b);
    void find_collision_points(const Rigidbody& rb_a, const Rigidbody& rb_b, CollisionResult& result);
    
    std::vector<float> project_vertices(const std::vector<Vec2>& verts, const Vec2& axis);
    std::vector<float> project_circle(const Vec2& center, const Vec2& axis, float radius);
    int find_closest_point_on_polygon(const Vec2& circle_center, const std::vector<Vec2>& trans_verts);
    CollisionResult point_segment_distance(const Vec2& point, const Vec2& seg_start, const Vec2& seg_end);
    
    std::vector<Vec2> find_circles_collision_point(const Vec2& center_a, float radius_a, 
                                                   const Vec2& center_b, float radius_b);
    std::vector<Vec2> find_polygons_collision_points(const std::vector<Vec2>& trans_verts_a,
                                                     const std::vector<Vec2>& trans_verts_b);
    std::vector<Vec2> find_circle_polygon_collision_point(const Vec2& circle_center, float circle_radius,
                                                          const Vec2& polygon_center, 
                                                          const std::vector<Vec2>& trans_verts);
    
    Vec2 calculate_line_intersection(const Vec2& p1, const Vec2& p2, const Vec2& p3, const Vec2& p4);
    bool is_point_in_line_segment(const Vec2& point, const Vec2& line_start, const Vec2& line_end);
    ...
    bool intersect_aabb(const AABB& aabb_a, const AABB& aabb_b)
    {
        return !(
    ...
    bool intersect_aabb_with_point(const AABB& aabb, const Vec2& point)
    {
    ...
    CollisionResult intersect_circle_polygon(const Rigidbody& circle_rb, const Rigidbody& polygon_rb)
    {
        float depth = FLT_MAX;
        Vec2 normal = {};
    ...
    Vec2 calculate_line_intersection(const Vec2& p1, const Vec2& p2, const Vec2& p3, const Vec2& p4)
    {
        ...
        float intersect_x = ((p1.m_x * p2.m_y - p1.m_y * p2.m_x) * (p3.m_x - p4.m_x) - 
                            (p1.m_x - p2.m_x) * (p3.m_x * p4.m_y - p3.m_y * p4.m_x)) / denominator;
        float intersect_y = ((p1.m_x * p2.m_y - p1.m_y * p2.m_x) * (p3.m_y - p4.m_y) - 
                            (p1.m_y - p2.m_y) * (p3.m_x * p4.m_y - p3.m_y * p4.m_x)) / denominator;
