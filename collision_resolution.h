...
namespace CollisionResolution
{
    ...
    inline void resolve_collision_linear(CollisionManifold &manifold)
    {
        ...
        if (relative_vel.dot(normal) > 0.0f)
            return;

        float inv_mass_a = rb_a.m_inv_mass;
        float inv_mass_b = rb_b.m_inv_mass;

        float j = -(1.0f + restitution) * relative_vel.dot(normal) / (inv_mass_a + inv_mass_b);

        Vec2 impulse = normal * j;

        rb_a.m_vel -= impulse * inv_mass_a;
        rb_b.m_vel += impulse * inv_mass_b;
    }

    inline void resolve_collision_rotation(CollisionManifold &manifold)
    {
        Rigidbody &rb_a = *manifold.m_rb_a;
        Rigidbody &rb_b = *manifold.m_rb_b;
        ...
        float e = std::min(rb_a.m_restitution, rb_b.m_restitution);

        std::vector<Vec2> impulse_list(manifold.m_contact_count);
        std::vector<Vec2> ra_list(manifold.m_contact_count);
        std::vector<Vec2> rb_list(manifold.m_contact_count);

        for (int i = 0; i < manifold.m_contact_count; ++i)
        {
            Vec2 ra = manifold.m_points_of_contact[i] - rb_a.m_pos;
            Vec2 rb = manifold.m_points_of_contact[i] - rb_b.m_pos;
            ...
            float j = -(1.0f + e) * contact_vel_mag;
            j /= denom;
            j /= static_cast<float>(manifold.m_contact_count);
